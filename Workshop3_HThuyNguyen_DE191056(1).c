...
int main(){
	...
	do{
		...
		printf("Your option is\t"); scanf("%d", &opt);
		switch (opt){
			case 1:{
				//income tax
				float ic, result;
				printf("---Calculating income tax program---\n");
				printf("Input your income per month: "); scanf("%f", &ic);
				result = cal_tax(ic);
				if (result == -1){
					printf("error!\n");
				}else{
			...
			case 2:{
				...
				printf("Input your number(integer) from keyboard: \t"); scanf("%d", &num);
				check_num(num, res);
				if (res[0] == 1){
			...
			case 3:{
				...
				printf("Input x & n from keyboard: \t"); scanf("%f%d", &a, &b);
				result = sum_of(a,b);
				if (result != -1){
